...
if [ $# -ne 1 ];
  then echo "Invalid number of arguments. You must pass in m"
  exit 1
fi
...
do 
  ./test_alg.sh CBRECURSION $normal_alg CHOOSE_STANDARD_SORTING $M 1
done
...
do
  ...
  do
    ./test_alg.sh  $interlaced_alg $interlaced_sort CHOOSE_INTERLACED_SORTING $M 1
  done
  echo " "
done
